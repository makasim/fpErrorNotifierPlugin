...
require_once(dirname(__FILE__).'/sfYamlInline12.class.php');
...
class sfYamlParser12
{
    ...
    while ($this->moveToNextLine())
    {
      ...
      if (preg_match('#^\-(\s+(?P<value>.+?))?\s*$#', $this->currentLine, $values))
      {
        ...
        else
        {
          ...
          {
            $data[] = array($matches[1] => sfYamlInline12::load($matches[2]));
          }
          else
          {
      ...
      {
        $key = sfYamlInline12::parseScalar($values['key']);
      ...
      else
      {
        ...
        if (1 == count(explode("\n", rtrim($this->value, "\n"))))
        {
          $value = sfYamlInline12::load($this->lines[0]);
          if (is_array($value))
          {
            $first = reset($value);
            if ('*' === substr($first, 0, 1))
  ...
  protected function parseValue($value)
  {
    ...
    {
      return sfYamlInline12::load($value);
    }
  }
